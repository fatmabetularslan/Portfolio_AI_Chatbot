...
if st.session_state.get('page') != 'chat':
    if st.button("📁  CV’yi Gör", key="cv_btn_tooldef", use_container_width=True):
        st.session_state['show_cv_options'] = True
        st.session_state['show_cv_preview'] = False

    if st.session_state.get('show_cv_options', False):
        st.markdown('<div class="cv-action-btn">', unsafe_allow_html=True)
        preview_clicked = st.button("👁️ CV'yi Önizle", key="cv_preview_btn", use_container_width=True)
        download_clicked = st.button("📥 PDF'i İndir", key="cv_download_btn", use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

        if preview_clicked:
            st.session_state['show_cv_preview'] = True
        ...
        if st.session_state.get('show_cv_preview', False):
            with open(PDF_PATH, "rb") as f:
                pdf_bytes = f.read()
            b64_pdf = base64.b64encode(pdf_bytes).decode("utf-8")
            st.markdown(
                f'''
                <div style='max-width: 740px; margin: 0 auto 32px auto; background: #fff; border-radius: 18px; box-shadow: 0 4px 32px 0 rgba(49,130,206,0.10); padding: 32px 24px 24px 24px; border: 1.5px solid #e3e8f0;'>
                    <div style='font-size:1.25em; font-weight:700; margin-bottom:18px; color:#1D3557;'>📄 Özgeçmiş Önizleme</div>
                    <iframe src="data:application/pdf;base64,{b64_pdf}" width="100%" height="600" style="border-radius:12px; border:1px solid #e3e8f0;"></iframe>
                    <div style='margin-top:18px; text-align:center;'>
                        <span style='font-size:0.98em; color:#555;'>CV’yi PDF olarak indir:</span><br>
                    </div>
                </div>
                ''',
                unsafe_allow_html=True,
